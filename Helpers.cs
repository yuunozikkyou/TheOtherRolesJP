       ...
       public static string GradientColorText(string startColorHex, string endColorHex, string text)
        ...
        public static void shareGameVersion() {
            try {
                ...
                if (pc == null || AmongUsClient.Instance == null) {
                    if (TheOtherRolesPlugin.DebugMode != null && TheOtherRolesPlugin.DebugMode.Value == "true")
                        TheOtherRolesPlugin.Logger.LogInfo("[TOR DEBUG] shareGameVersion skipped (LocalPlayer not ready)");
                    return;
                }

                MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(pc.NetId, (byte)CustomRPC.VersionHandshake, Hazel.SendOption.Reliable, -1);
                ...
                if (TheOtherRolesPlugin.DebugMode != null && TheOtherRolesPlugin.DebugMode.Value == "true")
                    TheOtherRolesPlugin.Logger.LogInfo($"[TOR DEBUG] shareGameVersion sent v{TheOtherRolesPlugin.Version} guid={Assembly.GetExecutingAssembly().ManifestModule.ModuleVersionId}");
            } catch (Exception e) {
                if (TheOtherRolesPlugin.DebugMode != null && TheOtherRolesPlugin.DebugMode.Value == "true")
                    TheOtherRolesPlugin.Logger.LogError($"[TOR DEBUG] shareGameVersion failed: {e}");
            }
        }
