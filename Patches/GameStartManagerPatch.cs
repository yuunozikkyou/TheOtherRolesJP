...
namespace TheOtherRoles.Patches
{
    public class GameStartManagerPatch
    {
        ...
        public class GameStartManagerUpdatePatch
        {
            ...
            private static bool loggedPostfixOnce = false;
            private static bool loggedPostfixErrorOnce = false;
            ...
            public static void Postfix(GameStartManager __instance)
            {
                try
                {
                    ...
                    if (__instance.StartButton == null || __instance.GameStartText == null || __instance.GameStartTextParent == null) return;

                    if (!loggedPostfixOnce && TheOtherRolesPlugin.DebugMode != null && TheOtherRolesPlugin.DebugMode.Value == "true")
                    {
                        loggedPostfixOnce = true;
                        TheOtherRolesPlugin.Logger.LogInfo($"[TOR DEBUG] GameStartManagerUpdatePatch.Postfix active v{TheOtherRolesPlugin.Version}");
                    }
                    ...
                    if (clients != null)
                    {
                        foreach (InnerNet.ClientData client in clients.ToArray())
                        {
                    ...
                    {
                        var pc = PlayerControl.LocalPlayer != null ? PlayerControl.LocalPlayer : CachedPlayer.LocalPlayer?.PlayerControl;
                        if (pc != null)
                        {
                            MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(pc.NetId, (byte)CustomRPC.SetGameStarting, Hazel.SendOption.Reliable, -1);
                    ...
                    if (CustomOptionHolder.anyPlayerCanStopStart.getBool() && copiedStartButton == null && __instance.GameStartText.text.StartsWith("Starting"))
                    {
                        ...
                        void StopStartFunc()
                        {
                            ...
                            if (pc != null)
                            {
                                MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(pc.NetId, (byte)CustomRPC.StopStart, Hazel.SendOption.Reliable, AmongUsClient.Instance.HostId);
                    ...
                    if (AmongUsClient.Instance.AmHost && sendGamemode && CachedPlayer.LocalPlayer != null)
                    {
                        ...
                        if (pc != null)
                        {
                            MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(pc.NetId, (byte)CustomRPC.ShareGamemode, Hazel.SendOption.Reliable, -1);
                ...
                {
                    if (!loggedPostfixErrorOnce && TheOtherRolesPlugin.DebugMode != null && TheOtherRolesPlugin.DebugMode.Value == "true")
                    {
                        loggedPostfixErrorOnce = true;
                        TheOtherRolesPlugin.Logger.LogError($"[TOR DEBUG] GameStartManagerUpdatePatch.Postfix exception: {e}");
                    }
                    return;
                }
            }
